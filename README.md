...
- 直接指定 md 文件时，会同时复制该文件所在目录下的其他文件和文件夹；指定的 md 文件会安装为 `SKILL.md`。
...
`Tab`/左右方向键切换页面，`↑`/`↓` 或 `j`/`k` 移动，`Enter` 选择/确认，`Space` 勾选目标，`r` 刷新，`u` 卸载，`b` 返回，`q` 退出。在市场详情页按 `u`，会卸载当前选中 md 文件对应的已安装目录。
